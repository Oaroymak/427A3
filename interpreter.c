...
// A3: Handle a page fault for the instruction at pcb->pc.
// Finds a free frame (or evicts the LRU frame if none available),
// loads the needed page, and prints the appropriate message.
// pc is NOT advanced – the instruction will be retried.
static void handle_page_fault(struct PCB *pcb) {
    ...
    if (free_frame >= 0) {
        ...
        printf("Page fault!\n");
        frame_load_page(pcb, page_idx, free_frame);
    } else {
        // Frame store is full – evict the LRU frame
        int victim = frame_find_lru();

        printf("Page fault! Victim page contents:\n\n");
        for (int i = 0; i < LINES_PER_PAGE; i++) {
            const char *line = frame_get_line(victim, i);
            if (line) printf("%s", line);
        }
        printf("\nEnd of victim page contents.\n");

        frame_evict(victim);
        frame_load_page(pcb, page_idx, victim);
    }
}

// see doc in header file
// A3: FCFS runs a process to completion; page faults load the needed page
// synchronously and immediately retry the same instruction (no re-queuing).
struct PCB *run_pcb_to_completion(struct PCB *pcb) {
    while (pcb_has_next_instruction(pcb)) {
        if (pcb_current_page_fault(pcb)) {
            handle_page_fault(pcb);
            continue; // retry the same pc – the page is now loaded
        }
...
// see doc in header file
// A3: For preemptive policies (RR), a page fault immediately ends the quantum.
// The page is loaded before putting the process back in the queue so that it
// will execute without faulting next time it is dequeued.
struct PCB *run_pcb_for_n_steps(struct PCB *pcb, size_t n) {
    debug("run n steps: n is %ld\n", n);
    for (; n && pcb_has_next_instruction(pcb); --n) {
        if (pcb_current_page_fault(pcb)) {
            handle_page_fault(pcb);
            // End the quantum immediately (return the pcb to the queue).
            // The page is now loaded; the instruction executes next quantum.
            return pcb;
        }
        parseInput(get_line(pcb_next_instruction(pcb)));
    }
